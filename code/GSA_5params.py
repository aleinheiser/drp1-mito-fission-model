...
def main():
    ...
    print(f"Running 5 parameter sensitivity analysis with {seed=}")

    with parallel_config(n_jobs=slots):
        res = run(seed, n)
        with open(f"{cache_dir}/result_{seed}_2^11_5params", "wb") as f:
            pickle.dump(res, f)


def run(seed, n):
    # The Saltelli sampler generates N*(2D+2) samples, where in this example N is 1024 and D is number of variables
    samples = sample(problem, n, seed=seed)  # makes a #x5 matrix
    np.savetxt(f"{cache_dir}/input_{seed}_2^11_5params", samples)
    rows = len(samples)
    ...
    np.savetxt(f"{cache_dir}/output_{seed}_2^11_5params", Y)
