...
def main():
    ...
    print(f"Running four parameter sensitivity analysis with {seed=}")

    with parallel_config(n_jobs=slots):
        res = run(seed, n)
        with open(f"{cache_dir}/result_{seed}_2^11_4params", "wb") as f:
            pickle.dump(res, f)
...
def run(seed, n):
    ...
    np.savetxt(f"{cache_dir}/input_{seed}_2^11_4params", samples)
    rows = len(samples)
    ...
    np.savetxt(f"{cache_dir}/output_{seed}_2^11_4params", Y)
